...
begin
  ...
  FListA.Values['Customer'] := Trim(EditName.Text);
  FListA.Values['Order'] := 'invtype,NPLANNUM ASC';

  if FOrderType = sFlag_DuanDao then
       nStr := GetQueryDispatchSQL(EncodeBase64(FListA.Text))
  else nStr := GetQueryOrderSQL(nStr, EncodeBase64(FListA.Text));
  ...
  begin
    ...
    if ((FOrderType = sFlag_Sale) or (FOrderType = sFlag_DuanDao)) and (FListA.Count > 0) then
    begin
