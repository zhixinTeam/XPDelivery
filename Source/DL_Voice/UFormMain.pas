...
begin
  ...
  try
    ...
    FTTSVoice := nIni.ReadInteger('Config', 'TTSVoice', 0) = 1;
    //TTS微软自带语音服务
  ...
  if FTTSVoice then
  begin
    gTTSVoiceManager.LoadConfig(gPath + 'TTSVoice.xml');
  end else
...
begin
  ...
  begin
    if CheckSrv.Checked then
         gTTSVoiceManager.StartVoice
    else gTTSVoiceManager.StopVoice;
  end else
  if FNetVoice then
  begin
    if CheckSrv.Checked then
    ...
    case nBase.FCommand of
     cVCCmd_PlaySound :
      begin
        ...
        if FTTSVoice then
          gTTSVoiceManager.PlayVoice(nStr) else
        if FNetVoice then
          gNetVoiceHelper.PlayVoice(nStr)  else
          gVoiceManager.PlayVoice(VoiceTextAdjust(nStr));
      end;
    end;
  end;
end;
